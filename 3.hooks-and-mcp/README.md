...
> 그 상태로는 실습 커밋이 수업 저장소에 섞이므로, 4번이 **그 연결을 끊는다.**
> 수업 저장소의 `.git` 이 `.git-backup` 으로 이름만 바뀌고(지우지 않는다),
> 이 폴더가 독립된 저장소가 된다. 되돌리려면 `.git-backup` 을 `.git` 으로 되돌리면 되고,
> 되돌리는 명령은 실행할 때 화면에 그대로 찍힌다.
> 끊은 뒤에는 `git pull` 로 자료를 다시 받지 않는다 — 실습은 이 폴더 하나로 끝난다.
